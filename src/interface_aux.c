 ...
 * This file implements a modal mask editor for the LiquidRescale plugin.
 *
 * Its purpose:
 * Main Function: Provides a separate editing mode where users can paint masks
 * (preservation, discard, or rigidity) on layers.
 *
 * Key Features:
 * Opens a simplified dialog with just an info message and OK button
 * Sets the target layer to 50% opacity for better visibility while painting
 * Sets appropriate foreground color based on mask type (green/red/blue)
 * Keeps the dialog window above others so users can paint while it's open
 * Restores original layer settings when done
 *
 * Workflow:
 *
 * User clicks "Edit" button in main dialog
 * This modal dialog opens with instructions
 * User paints on the layer using GIMP's paint tools
 * User clicks OK to return to main dialog
 * It's essentially a "paint mode" that temporarily modifies the layer state
 * to make mask editing easier, then cleans up when finished.
 *
 */
...
dialog_aux(
    ...
    g_assert (gimp_drawable_is_layer_id(ui_state->layer_on_edit_ID) == TRUE);
    ...
    gimp_image_set_active_layer_id(image_ID, ui_state->layer_on_edit_ID);
    gimp_layer_set_opacity_id(ui_state->layer_on_edit_ID, 50);
...
callback_dialog_aux_response(GtkWidget *dialog, gint response_id, gpointer data) {
    ...
    switch (response_id) {
        case GTK_RESPONSE_OK:
            gimp_image_set_active_layer_id(ia_data->image_ID, ia_data->layer_ID);
...
GeglColor *colour_from_type(gint32 image_ID, AuxLayerType layer_type) {
    switch (gimp_image_base_type_id(image_ID)) {
        case GIMP_RGB:
            switch (layer_type) {
                case AUX_LAYER_PRES:
