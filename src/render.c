...
#define IMAGE_TYPE_CHECK(image_ID, base_type) G_STMT_START { \
  if (gimp_image_base_type_id(image_ID) != base_type) \
    { \
...
#define UNFLOAT(layer_ID) unfloat_layer_id(layer_ID)

#define UNMASK(layer_ID) G_STMT_START { \
  if (gimp_layer_get_mask_id (layer_ID) != -1) \
    { \
      gimp_layer_remove_mask_id (layer_ID, vals->mask_behavior); \
    } \
  } G_STMT_END

#define SELECTION_SAVE(image_ID) G_STMT_START { \
  if (!gimp_selection_is_empty_id (image_ID)) \
    { \
      gimp_selection_save_id (image_ID); \
      gimp_selection_none_id (image_ID); \
      gimp_image_unset_active_channel_id (image_ID); \
    } \
  } G_STMT_END
...
static gboolean copy_aux_layer_to_new_image(gint32 image_ID, gint32 *layer_ID, gint x_off, gint y_off);
...
render_init_carver(PlugInImageVals *image_vals,
  ...
  layer_ID = drawable_vals->layer_ID;
    ...
    if (!layer_ID) {
        layer_ID = gimp_image_get_active_layer_id(image_ID);
    }
    ...
    if (!interactive) {
        ...
        if ((vals->output_seams) && (gimp_image_base_type_id(image_ID) != GIMP_RGB)) {
            gimp_image_convert_rgb_id(image_ID);
        }
    }

    if (vals->output_target == OUTPUT_TARGET_NEW_LAYER) {
        g_snprintf(new_layer_name, LQR_MAX_NAME_LENGTH, "%s LqR", layer_name);
        layer_ID = gimp_layer_copy_id(layer_ID);
        gimp_image_insert_layer_id(image_ID, layer_ID, 0, -1);
        gimp_drawable_set_name_id(layer_ID, new_layer_name);
        gimp_item_set_visible(GIMP_ITEM(gimp_drawable_get_by_id(layer_ID)), FALSE);
    } else if (vals->output_target == OUTPUT_TARGET_NEW_IMAGE) {
        image_ID = gimp_image_new_id(old_width, old_height, gimp_image_base_type_id(image_ID));
        ...
        if (vals->resize_aux_layers) {
            ...
            copy_aux_layer_to_new_image(image_ID, &vals->disc_layer_ID, x_off, y_off);
            copy_aux_layer_to_new_image(image_ID, &vals->rigmask_layer_ID, x_off, y_off);
            x_off = 0;
            y_off = 0;
        }
        gimp_display_new_id(image_ID);
        gimp_image_undo_group_end_id(image_ID);
    }
    ...
    carver_data->image_ID = image_ID;
    carver_data->layer_ID = layer_ID;
    carver_data->base_type = gimp_image_base_type_id(image_ID);
...
render_noninteractive(PlugInVals *vals,
    ...
    gimp_layer_set_lock_alpha_id(layer_ID, alpha_lock);
    if (vals->resize_aux_layers == TRUE) {
        if (vals->pres_layer_ID != 0) {
            gimp_layer_set_lock_alpha_id(vals->pres_layer_ID, alpha_lock_pres);
        }
        if (vals->disc_layer_ID != 0) {
            gimp_layer_set_lock_alpha_id(vals->disc_layer_ID, alpha_lock_disc);
        }
        if (vals->rigmask_layer_ID != 0) {
            gimp_layer_set_lock_alpha_id(vals->rigmask_layer_ID, alpha_lock_rigmask);
        }
    }
