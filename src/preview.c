...
preview_data_create(gint32 image_ID, gint32 layer_ID, PreviewData *p_data) {
    ...
    p_data->base_pixbuf = gimp_drawable_get_thumbnail_id(layer_ID, p_data->width, p_data->height,
...
//                    -1, -1, GDK_RGB_DITHER_NORMAL, 0, 0);
    ...
    GdkDrawingContext *ctx = gdk_window_begin_draw_frame(gtk_widget_get_window(p_data->area), NULL);
    cairo_t *cr = gdk_drawing_context_get_cairo_context(ctx);
    gdk_cairo_set_source_pixbuf(cr, p_data->pixbuf,
                                (PREVIEW_MAX_WIDTH - p_data->width) / 2,
                                (PREVIEW_MAX_HEIGHT - p_data->height) / 2);
    cairo_paint(cr);
    gdk_window_end_draw_frame(gtk_widget_get_window(p_data->area), ctx);
...
void
update_info_aux_use_icons(PlugInVals *vals, PlugInUIVals *ui_vals, GtkWidget *pres_use_image, GtkWidget *disc_use_image,
    ...
    if (ui_vals->pres_status == TRUE) {
        ...
        g_snprintf(help_text, MAX_STRING_SIZE, "%s %s", help_text_common, gimp_item_get_name_id(vals->pres_layer_ID));
    ...
    if (ui_vals->disc_status == TRUE) {
        ...
        g_snprintf(help_text, MAX_STRING_SIZE, "%s %s", help_text_common, gimp_item_get_name_id(vals->disc_layer_ID));
    ...
    if (ui_vals->rigmask_status == TRUE) {
        ...
        g_snprintf(help_text, MAX_STRING_SIZE, "%s %s", help_text_common,
                   gimp_item_get_name_id(vals->rigmask_layer_ID));
