...
count_extra_layers(gint32 image_ID) {
    ...
    gimp_image_get_layers_id(image_ID, &num_layers);
    return num_layers - 1;
}
...
//    if (image_ID != gimp_drawable_get_image_id(ref_layer_ID)) {
//        return FALSE;
//    }
...
dialog_layer_constraint_func(
    ...
    GimpImage *ref_image = gimp_item_get_image(GIMP_ITEM(ref_layer));
    if (image != ref_image) {
        return FALSE;
    }

    // Check if this is the same layer as the reference layer
    if (item == GIMP_ITEM(ref_layer)) {
        return FALSE;
    }
...
combo_get_active(GtkWidget *combo, PreviewData *p_data,
    ...
    if (status == TRUE) {
        ...
        gimp_drawable_get_offsets_id(*layer_ID_add, &(size_info->x_off), &(size_info->y_off));

        size_info->x_off -= p_data->x_off;
        size_info->y_off -= p_data->y_off;

        size_info->width = gimp_drawable_get_width_id(*layer_ID_add);
        size_info->height = gimp_drawable_get_height_id(*layer_ID_add);

        size_info_scale(size_info, p_data->factor);

        *pixbuf_add = gimp_drawable_get_thumbnail_id(*layer_ID_add, size_info->width, size_info->height,
...
callback_new_mask_button(GtkWidget *button, gpointer data) {
    ...
    switch (gimp_image_base_type_id(p_data->image_ID)) {
        case GIMP_RGB:
    ...
    gimp_image_undo_group_start_id(p_data->image_ID);
    layer_ID =
            gimp_layer_new_id(p_data->image_ID, nl_data->name,
                              p_data->old_width, p_data->old_height,
                              image_type, 50, GIMP_LAYER_MODE_NORMAL);


    gimp_image_insert_layer_id(p_data->image_ID, layer_ID, 0, -1);
    gimp_drawable_fill_id(layer_ID, GIMP_FILL_TRANSPARENT);
    gimp_layer_set_offsets(GIMP_LAYER(gimp_drawable_get_by_id(layer_ID)), p_data->x_off, p_data->y_off);
    gimp_image_undo_group_end_id(p_data->image_ID);
...
callback_edit_mask_button(GtkWidget *button, gpointer data) {
    ...
    gimp_image_undo_group_start_id(p_data->image_ID);
    gimp_image_set_active_layer_id(p_data->image_ID, layer_ID);
    gimp_image_undo_group_end_id(p_data->image_ID);
...
guess_new_size(GtkWidget *button, PreviewData *p_data, GuessDir direction) {
    ...
    gimp_drawable_get_offsets_id(disc_layer_ID, &x_off, &y_off);
